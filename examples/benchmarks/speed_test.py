...
async def main():
    ...
    connection = await asyncio_redis.Pool.create(host='localhost', port=6379,
                                                 poolsize=50)
    ...
    try:
        ...
        for i in range(10 * 1000):
            await connection.set('key',
                                 'value')  # By using await here, we wait for the answer.
        ...
        # Do 10,000 set requests
        futures = [asyncio.Task(connection.set('key', 'value')) for x in
                   range(10 * 1000)]
