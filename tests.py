...
class RedisProtocolTest(TestCase):
    ...
    async def test_extended_set(self, transport, protocol):
        ...
        value = await protocol.set(
            "my_key", "my_value", expire=10, only_if_not_exists=True
        )
        self.assertEqual(value, StatusReply("OK"))
        ...
        value = await protocol.set(
            "my_key", "other_value", pexpire=20000, only_if_exists=True
        )
        self.assertEqual(value, StatusReply("OK"))
    ...
    async def test_pubsub(self, transport, protocol):
        async def listener():
            ...
            self.assertEqual(
                repr(value), "PubSubReply(channel='our_channel', value='message2')"
            )
        ...
        async def sender():
            ...
            # Test pubsub numsub.
            result = await protocol.pubsub_numsub(
                ["our_channel", "some_unknown_channel"]
            )
            self.assertIsInstance(result, DictReply)
            result = await result.asdict()
            self.assertEqual(len(result), 2)
    ...
    async def test_zset(self, transport, protocol):
        ...
        self.assertEqual(
            (await result.asdict()), {"key": 4.0, "key2": 5.0, "key3": 5.5}
        )
        ...
        self.assertEqual((await result.asdict()), {"key2": 5.0, "key3": 5.5})
        result = await protocol.zrange("myzset", -2, -1)
        self.assertIsInstance(result, ZRangeReply)
        ...
        result = await protocol.zrangebyscore(
        ...
        self.assertEqual(
            (await protocol.zrevrangebyscore_asdict("myzset")),
        ...
        result = await protocol.zrevrangebyscore(
            "myzset", max=ZScoreBoundary(5.5, exclude_boundary=True)
        )
        self.assertEqual((await result.asdict()), {"key": 4.0, "key2": 5.0})
    ...
    async def test_zrevrange(self, transport, protocol):
        ...
        self.assertEqual(
            (await protocol.zrevrange_asdict("myzset")),
            {"key": 4.0, "key2": 5.0, "key3": 5.5},
        )
    ...
    async def test_zset_zincrby(self, transport, protocol):
        ...
        result = await protocol.zrange("myzset")
        self.assertEqual(
            (await result.asdict()), {"key": 5.1, "key2": 5.0, "key3": 5.5}
        )
    ...
    async def test_lua_script(self, transport, protocol):
        ...
        # Test script exists
        result = await protocol.script_exists(
...
class RedisConnectionTest(TestCase):
    ...
    async def test_connection(self):
        ...
        self.assertEqual(repr(connection), f"Connection(host='{HOST}', port={PORT})")
    ...
    async def test_parallel_requests(self):
        ...
        async def sink():
            for i in range(0, 5):
                ...
                self.assertIn(
                    "BlockingPopReply(list_name='my-list', value='", repr(reply)
                )
    ...
    def test_no_global_loop(self):
        ...
        try:
            ...
            connection = new_loop.run_until_complete(
                Connection.create(host=HOST, port=PORT)
            )
